        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h3 class="card-title">Edit Property Card</h3>
                    <a href="{{ route('property_cards.index') }}" class="btn btn-secondary">
                        <i class="fas fa-arrow-left"></i> Back to List
                    </a>
                </div>
                
                <form action="{{ route('property_cards.update', $property_cards->property_card_id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        @if ($errors->any())
                                    ...
                                    <select name="received_equipment_item_id" id="received_equipment_item_id" class="form-select @error('received_equipment_item_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($receivedEquipmentItems as $item)
                                            <option value="{{ $item->item_id }}" 
                                                    ...
                                                    {{ old('received_equipment_item_id', $property_cards->received_equipment_item_id) == $item->item_id ? 'selected' : '' }}>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select name="locations_id" id="locations_id" class="form-select @error('locations_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($locations as $location)
                                            <option value="{{ $location->id }}" {{ old('locations_id', $property_cards->locations_id) == $location->id ? 'selected' : '' }}>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="row">
                                            <div class="col-md-3">
                                                ...
                                                <div id="display-entity">{{ $property_cards->entity_name ?? 'N/A' }}</div>
                                            </div>
                                            <div class="col-md-3">
                                                <strong>Property No:</strong>
                                                <div id="display-property-no">{{ $property_cards->property_number ?? 'N/A' }}</div>
                                            </div>
                                            <div class="col-md-3">
                                                <strong>Amount:</strong>
                                                <div id="display-amount">₱{{ number_format($property_cards->amount ?? 0, 2) }}</div>
                                            </div>
                                            <div class="col-md-3">
                                                <strong>Date Acquired:</strong>
                                                <div id="display-date-acquired">{{ $property_cards->date_acquired ? \Carbon\Carbon::parse($property_cards->date_acquired)->format('M d, Y') : 'N/A' }}</div>
                                            </div>
                                        </div>
                                        <div class="row mt-2">
                                            <div class="col-12">
                                                <strong>Description:</strong>
                                                <div id="display-description">{{ $property_cards->description ?? 'N/A' }}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" name="article" id="article" class="form-control @error('article') is-invalid @enderror" 
                                           value="{{ old('article', $property_cards->article) }}" required>
                                    @error('article')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="number" name="qty_physical" id="qty_physical" class="form-control @error('qty_physical') is-invalid @enderror" 
                                           value="{{ old('qty_physical', $property_cards->qty_physical) }}" min="0" required>
                                    @error('qty_physical')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="number" name="balance" id="balance" class="form-control @error('balance') is-invalid @enderror" 
                                           value="{{ old('balance', $property_cards->balance) }}" step="0.01" min="0">
                                    @error('balance')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <select name="condition" id="condition" class="form-select @error('condition') is-invalid @enderror" required>
                                        ...
                                        <option value="New" {{ old('condition', $property_cards->condition) == 'New' ? 'selected' : '' }}>New</option>
                                        <option value="Good" {{ old('condition', $property_cards->condition) == 'Good' ? 'selected' : '' }}>Good</option>
                                        <option value="Fair" {{ old('condition', $property_cards->condition) == 'Fair' ? 'selected' : '' }}>Fair</option>
                                        <option value="Poor" {{ old('condition', $property_cards->condition) == 'Poor' ? 'selected' : '' }}>Poor</option>
                                        <option value="For Repair" {{ old('condition', $property_cards->condition) == 'For Repair' ? 'selected' : '' }}>For Repair</option>
                                        <option value="Condemned" {{ old('condition', $property_cards->condition) == 'Condemned' ? 'selected' : '' }}>Condemned</option>
                                    </select>
                                    @error('condition')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" name="received_by_name" id="received_by_name" class="form-control @error('received_by_name') is-invalid @enderror" 
                                           value="{{ old('received_by_name', $property_cards->received_by_name) }}">
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <textarea name="issue_transfer_disposal" id="issue_transfer_disposal" class="form-control @error('issue_transfer_disposal') is-invalid @enderror" 
                                              rows="3">{{ old('issue_transfer_disposal', $property_cards->issue_transfer_disposal) }}</textarea>
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <textarea name="remarks" id="remarks" class="form-control @error('remarks') is-invalid @enderror" 
                                              rows="3">{{ old('remarks', $property_cards->remarks) }}</textarea>
                                    @error('remarks')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                    ...
                    <div class="card-footer">
                        <button type="submit" class="btn btn-success">
                            <i class="fas fa-save"></i> Update Property Card
                        </button>
                        <a href="{{ route('property_cards.index') }}" class="btn btn-secondary">
